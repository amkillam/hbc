...
view *browser_init(void) {
  v_browser = view_new(AE_OFFSET + (MAX_COLUMNS * ROWS * 2), NULL, 0, 0, 0, 0);
...
void browser_theme_reinit(void) {
  ...
  widget_image(&v_browser->widgets[0], 24, 192, 0, theme_gfx[THEME_ARROW_LEFT],
               NULL, true, theme_gfx[THEME_ARROW_LEFT_FOCUS]);
  ...
  widget_image(&v_browser->widgets[3],
               ...
               theme_gfx[THEME_LAN_ACTIVE], theme_gfx[THEME_LAN], false, NULL);
...
void browser_gen_view(browser_action action, const app_entry *app) {
  ...
  if (columns_current == 1)
    ...
    x1 = (view_width - (theme_gfx[THEME_GRID_APP_ENTRY]->w * columns_current)) /
         2;

  if (columns_new == 1)
    x2 = (view_width - theme_gfx[THEME_APP_ENTRY]->w) / 2;
  else
    x2 = (view_width - (theme_gfx[THEME_GRID_APP_ENTRY]->w * columns_new)) / 2;
  ...
  if (action != BA_REMOVE)
    for (i = 0; i < (columns_new * ROWS); ++i) {
      if (entry_count > top_offset + i) {
        ...
        if (columns_new == 1)
          widget_app_entry(&v_browser->widgets[o2 + i], x2, y, 0, entry);
        else
          widget_grid_app_entry(
              &v_browser->widgets[o2 + i],
              x2 + ((i % columns_new) * theme_gfx[THEME_GRID_APP_ENTRY]->w), y,
              0, entry);
      }
  ...
  for (i = 0; i < TRANS_STEPS; ++i) {
    ...
    for (j = 0; j < MAX_COLUMNS * ROWS; ++j) {
      if ((i > j / columns_current) &&
        ...
        val1[j] += step;
      if ((i > j / columns_new) && (i < TRANS_STEPS - (ROWS - j / columns_new)))
        val2[j] += step;
      ...
      v_browser->widgets[o2 + j].coords.x =
          x2 - f2 + ((j % columns_new) * theme_gfx[THEME_GRID_APP_ENTRY]->w);
    }
